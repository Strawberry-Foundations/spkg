...
class PluginHandler:
    def setup():
        ...
        os.system(f"sudo debootstrap --arch={arch} --variant=minbase --include=systemd,libsystemd0,wget,ca-certificates,busybox-static {dist} {bootstrap_location} {repo}")
        
        print(f"{Fore.YELLOW + BOLD}[!]{Fore.RESET + RESET} Updating your sandbox ...")
        os.system(f"sudo chroot {bootstrap_location} apt clean all &> /dev/null && apt autoclean &> /dev/null && apt update &> /dev/null")
        
        print(f"{Fore.YELLOW + BOLD}[!]{Fore.RESET + RESET} Modifying /etc/apt/sources.list for the best program compability")
        os.system(f"sudo rm {bootstrap_location}/etc/apt/sources.list")
        
        if arch == "x86_64" or arch == "x86":
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://archive.ubuntu.com/ubuntu {dist} main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://archive.ubuntu.com/ubuntu {dist}-backports main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://archive.ubuntu.com/ubuntu {dist}-proposed main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://archive.ubuntu.com/ubuntu {dist}-security main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://archive.ubuntu.com/ubuntu {dist}-updates main restricted universe multiverse"')
            
        else:   
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://ports.ubuntu.com/ubuntu-ports {dist} main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://ports.ubuntu.com/ubuntu-ports {dist}-backports main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://ports.ubuntu.com/ubuntu-ports {dist}-proposed main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://ports.ubuntu.com/ubuntu-ports {dist}-security main restricted universe multiverse"')
            os.system(f'sudo tee -a {bootstrap_location}/etc/apt/sources.list <<< "deb http://ports.ubuntu.com/ubuntu-ports {dist}-updates main restricted universe multiverse"')  
            
        print(f"{Fore.YELLOW + BOLD}[!]{Fore.RESET + RESET} Installing some base packages ... ")
        os.system(f"sudo chroot {bootstrap_location} apt update &> /dev/null")
        os.system(f"sudo chroot {bootstrap_location} apt install -y python3 python3-dev python3-pip #&> /dev/null")
        
        end_time = time.time()
        
        print(f"{Fore.GREEN + BOLD}[!]{Fore.RESET + RESET} Finished sandbox setup in {round(start_time - end_time, 2)} s")
